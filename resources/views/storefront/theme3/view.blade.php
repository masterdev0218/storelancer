                ...
                <div class="breadcrumb-section">
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a
                                href="{{ route('store.slug', $store->slug) }}">{{ __('Main site') }}</a>
                        </li>
            ...
            <div class="row row-grid">
                ...
                <div class="col-lg-6">
                            ...
                            @if (Auth::guard('customers')->check())
                                @if (!empty($wishlist) && isset($wishlist[$products->id]['product_id']))
                                    @if ($wishlist[$products->id]['product_id'] != $products->id)
                                        <button type="button"
                                            class="btn btn-sm btn-primary rounded-pill btn-icon hover-translate-y-n3 add_to_wishlist wishlist_{{ $products->id }}"
                                            data-id="{{ $products->id }}">
                                            <i class="far fa-heart"></i>
                                        </button>
                                    @else
                                        <button type="button"
                                            class="btn btn-sm btn-primary rounded-pill btn-icon hover-translate-y-n3 bg-light-gray"
                            ...
                            @foreach ($product_variant_names as $key => $variant)
                                <div class="col-sm-6 mb-4 mb-sm-0">
                                    ...
                                    <select name="product[{{ $key }}]" id="pro_variants_name"
                                        class="form-control custom-select variant-selection  pro_variants_name{{ $key }} pro_variants_name variant_loop variant_val">
                    ...
                    <div class="cat-id d-flex">
                        <p class="mb-0 t-black14 f-14"><span class="t-l-gray">{{ __('Category') }}:</span>
                            {{ $products->product_category() }}</p>
                        <p class="mb-0 t-black14 ml-4 f-14"><span class="t-l-gray">{{ __('ID') }}: </span>
                            {{ $products->SKU }}</p>
                    </div>
                                ...
                                <div class="card card-product">
                                        ...
                                        <a href="{{ route('store.product.product_view', [$store->slug, $product->id]) }}">
                                            @if (!empty($product->is_cover) )
                                                <img alt="Image placeholder" src="{{ $proimg . $product->is_cover }}"
                                                    class="img-center img-fluid" style="width:auto; height:221px;">
                                            @else
                                    ...
                                    <div class="card-body pt-0">
                                        @if ($product['enable_product_variant'] == 'on')
                                            <h3 class="store-title-medium t-secondary ml-5 mb-4">{{ __('In Variant') }}
                                            </h3>
                                            <a class="btn btn-sm btn-primary  btn-icon hover-translate-y-n3"
                                                href="{{ route('store.product.product_view', [$store->slug, $product->id]) }}">
                                                <span class="btn-inner--icon">
                                                    {{ __('ADD TO CART') }}
                                                    <i class="fas fa-shopping-basket"></i>
                                            ...
                                            @if (Auth::guard('customers')->check())
                                                @if (!empty($wishlist) && isset($wishlist[$product->id]['product_id']))
                                                    @if ($wishlist[$product->id]['product_id'] != $product->id)
                                                        <button type="button"
                                                            class="btn btn-sm btn-primary  btn-icon hover-translate-y-n3 add_to_wishlist wishlist_{{ $product->id }}"
                                                            data-id="{{ $product->id }}">
                                                            <i class="far fa-heart"></i>
                                                        </button>
                                                    @else
                                                        <button type="button"
                                                            class="btn btn-sm btn-primary  btn-icon hover-translate-y-n3 bg-light-gray"
                                            ...
                                            @endif
                                        @else
                                            <h3 class="store-title-medium t-secondary ml-5 mb-4">
                                                {{ \App\Models\Utility::priceFormat($product->price) }}</h3>
                                            <a class="btn btn-sm btn-primary  btn-icon hover-translate-y-n3"
                                                href="{{ route('store.product.product_view', [$store->slug, $product->id]) }}">
                                                <span class="btn-inner--icon">
                                                    {{ __('ADD TO CART') }}
